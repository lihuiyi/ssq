...
def kill_rule(kill_data):
    ...
    for column in kill_data.columns:
        ...
        condition_1_data = kill_data.loc[kill_data[column] > 33, column]
        ...
        kill_data.loc[condition_2_index, column] = 10  # 被杀号码大于33，并且尾数为0时，杀号码10
        kill_data.loc[condition_3_index, column] = condition_1_data % 10  # 被杀号码大于33，并且尾数不为0时，杀号码尾数
    return kill_data
...
def method_P(red_num, blue_num):
    P1_data = blue_num + 2
    P2_data = blue_num + red_num["red6"]
    P3_data = blue_num + red_num["red2"] - 1
    P4_data = (34 - red_num["red1"]) * 0.88
    # 把 O7_data 四舍五入保留整数，然后把 float 转换为 int
    P4_data = round(P4_data, 0).apply(pd.to_numeric, downcast="integer")
    P5_data = 34 - red_num["red3"] + 7
    P6_data = (red_num["red1"] * 4) -2
    P7_data = red_num["red4"] - red_num["red1"] + 7
    P8_data = red_num["red5"] - red_num["red4"] + 10
    P9_data = red_num["red6"] - red_num["red2"] + 10
    P10_data = (red_num["red2"] + red_num["red3"] + red_num["red4"] + red_num["red5"] + blue_num) / 6
    # 把 O17_data 四舍五入保留整数，然后把 float 转换为 int
    P10_data = round(P10_data, 0).apply(pd.to_numeric, downcast="integer")
    P11_data = (red_num["red3"] + red_num["red4"] + red_num["red5"] + red_num["red6"]) / 4
    # 把 O18_data 四舍五入保留整数，然后把 float 转换为 int
    P11_data = round(P11_data, 0).apply(pd.to_numeric, downcast="integer")
    # 把 O1_data 到 O18_data 转换为 DataFrame
    P1_data = pd.DataFrame(P1_data.values, columns=["P1"])
    P2_data = pd.DataFrame(P2_data.values, columns=["P4"])
    P3_data = pd.DataFrame(P3_data.values, columns=["P5"])
    P4_data = pd.DataFrame(P4_data.values, columns=["P7"])
    P5_data = pd.DataFrame(P5_data.values, columns=["P8"])
    P6_data = pd.DataFrame(P6_data.values, columns=["P10"])
    P7_data = pd.DataFrame(P7_data.values, columns=["P13"])
    P8_data = pd.DataFrame(P8_data.values, columns=["P15"])
    P9_data = pd.DataFrame(P9_data.values, columns=["P16"])
    P10_data = pd.DataFrame(P10_data.values, columns=["P17"])
    P11_data = pd.DataFrame(P11_data.values, columns=["P18"])
    # method_O()函数中全部的杀号方法
    kill_data = pd.concat(
        [P1_data, P2_data, P3_data, P4_data, P5_data, P6_data, P7_data, P8_data, P9_data, P10_data, P11_data],
        axis=1
    )
    kill_data = kill_rule(kill_data)
    kill_data = kill_data.drop(kill_data.index[-1], axis=0)
...
def kill_number(red_num, blue_num):
    ...
    method_P_data = method_P(red_num, blue_num)
    data_list = [
        method_A_data, method_B_data, method_C_data, method_D_data, method_E_data, method_F_data, method_G_data,
        method_H_data, method_I_data, method_J_data, method_K_data, method_L_data, method_N_data, method_P_data
    ]
...
def judge_right_or_not(red_num, kill_data):
    """
    功能：判断每一种杀号方法是对还是错
    参数：
        red_num：红球
    ...
    for column in kill_data.columns:
        ...
        for i in range(kill_data.shape[0]):
            # 计算杀号方法有没有杀对号码
            is_not_in = column_data.loc[i] not in red_num.loc[i].as_matrix()
            is_nan = False
            if i == 0 or i == 1:
...
def to_execl(correct_or_wrong_data, accuracy_data, output_dir):
    ...
    output_path1 = os.path.join(output_dir, "红球杀号对错_label.xlsx")
    output_path2 = os.path.join(output_dir, "红球杀号准确率.xlsx")
...
output_dir = os.path.join(data_dir, "train_data")
# 读取数据
...
# 判断每一种杀号方法是对还是错
correct_or_wrong_data = judge_right_or_not(red_num, kill_data)
# 计算每一种杀号方法的准确率
