...
# 训练多个模型
time_step = 14
label_name_list = ["red5_label", "red6_label", "blue_label"]
hidden_layer_units_list = [128, 128, 100, 100, 156, 156, 128]

for i in range(len(label_name_list)):
    for j in range(len(hidden_layer_units_list)):
        ...
        hidden_layer_units = hidden_layer_units_list[j]  # 隐藏层单元数
        ...
        train_epochs = 300
        epochs_per_eval = 5
        model_dir = r"F:\SSQ" + "\\" + str(time_step) + "期" + "\\" + label_name + "\\" + label_name + "_model" + str(j + 1)
        ...
        fit(
            ...
            weight_decay, learning_rate, keep_prob, epochs_per_eval, model_dir, scope_name=label_name + "_model" + str(j + 1)
        )
